      ...
      def teams_progress
        ...
        unless selectedEvent
          ...
          if (Event.where('id in (?)', selectedEvent).count>0)
            ...
            respond_with Team.find_by_sql("SELECT teams.id AS team_id, sum(activities.distance) AS progress, teams.team_goal FROM teams INNER JOIN registrations ON teams.id=registrations.team_id INNER JOIN activities ON registrations.user_id = activities.user_id WHERE teams.event_id = " + selectedEvent + " AND activities.event_id = " + selectedEvent + " GROUP BY teams.id")
          else
            respond_with nil
          end
        end
